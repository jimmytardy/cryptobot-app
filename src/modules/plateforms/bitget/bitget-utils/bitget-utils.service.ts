...
export class BitgetUtilsService {
    ...
    async getQuantityForOrder(client: FuturesClient, user: User) {
        let { quantity, pourcentage } = user.preferences.bot || {}
    ...
    caculateTPsToUse(tps: number[], size: number, TPSize: TPSizeType, symbolRules: FuturesSymbolRule, sideOrder: FuturesHoldSide, currentPrice?: number): { TPPrice: number[]; TPSize: number[] } {
